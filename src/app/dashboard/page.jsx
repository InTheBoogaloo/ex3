...
export default function DashboardPage() {
  ...
  const initials = student
    ? (student.nombre || student.name || 'U').split(' ').slice(0, 2).map(w => w[0]).join('').toUpperCase()
    : '?';
    ...
    <section className={styles.page}>
      ...
      <div className={styles.hero}>
        ...
        <div className={styles.heroInfo}>
          {loading
            ? <><Skeleton w="220px" h="1.4rem" /><Skeleton w="140px" h=".85rem" /></>
            : <>
                <h2 className={styles.studentName}>{student?.nombre || student?.name || 'Estudiante'}</h2>
                <span className={styles.studentId}>{student?.numero_control || student?.matricula || ''}</span>
              </>
          }
        </div>
        <div className={styles.statusBadge}>
          {loading ? <Skeleton w="80px" h="1.4rem" r="20px" /> : <span className={styles.badge}>Activo</span>}
        </div>
      </div>
